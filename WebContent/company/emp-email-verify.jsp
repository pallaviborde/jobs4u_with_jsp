...
<%
    ...
    //ResultSet rs=con.getRs("SELECT * FROM Recruiter WHERE email='"+strEmail+"'");
    //out.print(strEmail);
    if(!(strEmail.matches("[a-zA-Z0-9\\.]+@[a-zA-Z0-9\\-\\_\\.]+\\.[a-zA-Z0-9]{3}"))){
            out.print("<script>alert('Please provide correct email format');document.location='emp-email-verify.html';</script>");
            return;
    	}/*else if (!strEmail.contains("itu.edu")){
    		out.print("<script>alert('Please use ITU email only to create your account.');document.location='emp-email-verify.html';</script>");
            return;
    	}*/
	if (strEmail.equals("")) {
		out.println("<script>alert('Please fill out the form completely.');document.location='emp-email-verify.html';</script>");
		return;
	} else {
		int intT=0;
		...
		if(intT>0){
			...
			//Data data = new Data();
			int intUpdate = 0;
			intUpdate = data.insert("UPDATE Recruiter SET password='"
					+ cutKey +"'WHERE recEmail='"
					+ strEmail + "'");
			if (intUpdate <= 0) {
				out.print("<script>alert('Your account update failed.');document.location='emp-email-verify.jsp';</script>");
				return;
			} /*else {
   ...
   //out.print("<script>alert(result);document.location='stu-email-verify.jsp';</script>");
		        ...
		        out.print("<script>alert('Incorrect Email or Password.Please try again.');document.location='emp-email-verify.html';</script>");
		        return;
		}
	} 
